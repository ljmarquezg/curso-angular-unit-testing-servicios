...
describe('PersonComponent', () => {
  ...
  it('should have <p> with "Mi altura es {person.height}"', () => {
    ...
    // Act
    fixture.detectChanges();
    const textBtn = getText(fixture, 'p');
    // Assert
    expect(textBtn).toEqual(expectedMsg);
    expect(textBtn).toContain(`${component.person.height}`);
  });
  ...
  it('should update button text IMC calculate IMC', () => {
    ...
    fixture.detectChanges();
    // Assert
    expect(getText(fixture, 'btn-imc')).toContain(expectedMsg);
  });
  ...
  it('should calculate IMC when clicking calculate button', () => {
    ...
    component.person = new Person('Valentina', 'Pérez', 25, 80, 1.75); // Overweigh
    // const debugElement: DebugElement = fixture.debugElement;
    const btnDebug = query(fixture, 'button.btn-imc');
  ...
  describe('onClick', () => {
    it('should raise selected event when do click', (doneFn) => {
      ...
      const buttonDebug = query(fixture, '.btn-choose');
